...
- [ ] `pnpm harness:load` (부하 베이스라인 — `next build && pnpm start` 선행. p50/p95/에러율 + 무료 한도 외삽. **베타 직전 1회 권고, CI 게이트 아님** — flaky + 한도 소진 위험)
...
## 윤리 / 투명성 (P3)
- [ ] 어트리뷰션 코드가 알고리즘 순위에 영향 없음 (단위 테스트 확인 — `src/engine/compare.isolation.test.ts`)
