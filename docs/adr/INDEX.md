...
**영향**: PLAN 4.6 본문 가이드라인 정합. 신설 파일 = `docs/marketing/beta-recruitment-copy.md` (scribe, 카피 4개 초안). PLAN §D.3 GATE-K 선행 작업(D.3.a/c/d) 명시. PLAN scope cut 옵션 E "적용 예약" 표 행 추가. ADR-0009 (2개 공급사) + ADR-0026 §T7 (`?ref=`) + 헌법 P3/§8 #1 cross-ref. 외부 의존성 0건. 코드/마이그레이션 0건. **회귀 트리거**: (1) 카피 4개 중 과장 토큰 발견 → Amendment 1 즉시 (2) 모집 URL 에 UTM 발견 → T5 위반 (3) 베타 폼 `/beta-signup` 추가 → T4 위반 (4) 3주 누적 >100명 → scope cut E 미발동 평가 (5) 피드백 채널 0개 수신 → 4.7 신호 부족.
