...
- [ ] **4.5** 전환 후 7일 이내 후속 메일 (선택 동의) — **ADR-0028** (architect, 2026-05-13 분해)
  ...
  - [x] **4.5.e** Unsubscribe 1-click — `src/app/unsubscribe/[token]/page.tsx` RSC 신설 (75줄). GET 으로 `unsubscribeToken` 매칭 → atomic UPDATE: `unsubscribed_at = now()` + `email = NULL` + `pii_anonymized_at = COALESCE(기존, now())`. 응답: 간결 confirmation 페이지 (다크패턴 0 — 재구독 유도/Confirmshaming/마케팅톤 0). Discriminated union `UnsubscribeResult` (not-found/already-unsubscribed/just-unsubscribed). idempotency: 재클릭도 동일 메시지 (상태 차이 노출 X). 토큰 형식 검증: nanoid 16자 `/^[A-Za-z0-9_-]{16}$/`. 헌법 §8 #1 자가(headers/cookies 0건). 4.5.b/c/d 회귀 X. typecheck/lint/test 445 passed (425+20) / harness:plan 51 정합 / harness:data 통과. ✅ 2026-05-13.
