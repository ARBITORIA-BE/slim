...
- [ ] **4.5** 전환 후 7일 이내 후속 메일 (선택 동의) — **ADR-0028** (architect, 2026-05-13 분해)
  ...
  - [x] **4.5.f** legal 에이전트 1차 검토 — GDPR Art. 6(1)(a) 동의 + Art. 7(3) 철회 + Art. 13 정보 제공 + 다크패턴 0 + 보존 정책. `docs/legal/gdpr-register.md` 에 새 처리 활동(후속 메일 발송) 등재. 4.1.d 인터스티셜 카피와 *일관성* 확인. 외부 변호사 감사 항목은 베타 직전/M16.
    - ✅ 완료 (2026-05-13): `docs/legal/gdpr-register.md` PA-05 신설 (후속 메일 발송 처리 활동). `docs/adr/0028-follow-up-email.md` §Legal Review §Verification A~I 9항목 판정 (8통과/1조건부 — Day 90 행 삭제 cron + Resend DPA). 잔존 조건 2건: Day 90 cron (4.5.f 후속 태스크), Resend DPA (외부 감사 항목 8번). `docs/adr/0026-affiliate-click-and-attribution.md` 외부 감사 표 항목 8번 신규 추가(Resend DPA). §Decision 무수정(append). 카피 일관성(4.1.d vs 4.5.c) 검증 완료. 커밋 진행 중.
